...
public class MandelbrotImage {
	...
	public static long calculatePoint(double xpos, double ypos, long limit) {
	    double zx = 0.0;
	    double zy = 0.0;
	    long count = 0;
	    while (count < limit) {
	...
	// Returns:
	//   void
	public static void calculateImage(long image[], int imagewidth, double centerx, double centery, double zoom, long limit) {
		...
		long time = end - start;
		double elapsed = (double)time/1_000_000_000;
    ...
    public static void main (String[] args) throws IOException {
		...
		System.out.println("------------ WELCOME TO MANDELBROT SET BENCHMARKING! ------------");
		System.out.println("------------- Jacob Hreshchyshyn SER450 - Project 2 -------------\n");
	    // allocate memory for the image
	    long[] image = new long[width * width];

	    // call routine to create the image
	    System.out.println("calculating the image");
	    calculateImage(image, width, 0.0, 0.0, 2.0, 10000);
	    
	    // output the image to a file
	    System.out.println("writing the image file");
        FileWriter ofile;
        ofile = new FileWriter("output.csv", true);
	    for (int row = 0;row < width;row++) {
	        for (int column=0;column<width;column++) {
				ofile.write(Long.toString(image[row * width + column]));
	            if (column + 1 < width) ofile.write(",");
	        }
	        ofile.write("\r\n");
	    }
	    ofile.close();
