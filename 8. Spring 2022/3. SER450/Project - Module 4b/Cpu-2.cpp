...
//********************************************
// Constructor
// initialize the cpu object - clear all the pipeline
Cpu::Cpu()
{
...
void Cpu::thread_ex_start() {
    ...
    //****************************************************
    // Forwarding Unit
    // Logic for ForwardA and ForwardB can be found in the 
    // class text on pages 308 and 311
    // 
    // Variable name substitutions from the text to this code
    // are as follows:
    // MEM/WB.RegWrite   ==> wb_regWrite
    // MEM/WB.RegisterRd ==> wb_registerNum
    // EX/MEM.RegWrite   ==> mem_regWrite
    // EX/MEM.RegisterRd ==> mem_registerNum
    // ID/EX.RegisterRs  ==> rsidx;
    // ID/EX.RegisterRt  ==> rtidx;
    unsigned int forwardA = 0;
    unsigned int forwardB = 0;
    // MEM Hazard
    if (wb_regWrite && (wb_registerNum != 0) && wb_registerNum == rsidx) {
        forwardA = 1;
    }
    if (wb_regWrite && (wb_registerNum != 0) && wb_registerNum == rtidx) {
        forwardB = 1;
    }
    // EX Hazard
    if (mem_regWrite && (mem_registerNum != 0) && mem_registerNum == rsidx) {
    ...
    if (forwardA == 1) {
        ...
        std::cout << "Forwarding RS from WB to ALU input." << std::endl;
    } 
    if (forwardA == 2) {
        operand1 = mem_aluResult;
        std::cout << "Forwarding RS from MEM to ALU input." << std::endl;
    }

    unsigned int dat2 = regRt;
    if (forwardB == 1) {
        dat2 = wb_regWrData;
        std::cout << "Forwarding RT from WB to ALU input." << std::endl;
    }
    if (forwardB == 2) {
        dat2 = mem_aluResult;
        std::cout << "Forwarding RT from MEM to ALU input." << std::endl;
    }
