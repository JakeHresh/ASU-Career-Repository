/**
* Chunk based decryption program that has a set of strings that can be
* displayed, sorted, and decrypted. The message: "C is quirky, flawed, and an enormous success." - Dennis M. Ritchie
*
...
void sort_chunks() {
	...
	int j;
	int min;
	for(i; i < NUMBER_OF_CHUNKS - 1; i++)
    {
	    min = i;
	    for(j = i + 1; j < NUMBER_OF_CHUNKS; j++)
        {
	        if(chunks[j][0] < chunks[min][0])
            {
	            min = j;
            }
...
void decrypt_chunks() {
	...
	int i = 0;
	int j;
	for(i; i < NUMBER_OF_CHUNKS; i++)
    {
	    char* activeChunk = chunks[i];
	    for(j = 1; j < CHUNK_LENGTH - 1; j++)
        {
	        *(activeChunk + j) -= DECRYPTION_SHIFT;
        }
    }
}
